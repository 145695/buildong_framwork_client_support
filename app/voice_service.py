...
async def startup():
    ...
    print(f"[Voice Service] Models: {list(ml_models.keys())}")
...
async def speech_to_text(audio: UploadFile = File(...)):
    ...
    temp_input = os.path.join(tempfile.gettempdir(), f"stt_in_{uuid.uuid4().hex}")
    temp_wav = os.path.join(tempfile.gettempdir(), f"stt_out_{uuid.uuid4().hex}.wav")
    ...
    try:
        ...
        # Convert to WAV using ffmpeg (handles any format: webm, mp3, wav, etc.)
        ...
        if not os.path.exists(temp_wav) or os.path.getsize(temp_wav) < 100:
            raise HTTPException(500, f"Audio conversion failed: {result.stderr[:200]}")
        
        # Riva clients already cloned in Dockerfile
        transcribe_script = "/app/python-clients/scripts/asr/transcribe_file_offline.py"
...
async def translate_text(req: TranslateRequest):
    try:
        ...
        print(f"[Voice] Translate: {req.source_language}→{req.target_language}")
        return {"translated_text": translated, "safety_label": safety}
    except Exception as e:
        print(f"[Voice] Translation fallback: {e}")
...
async def text_to_speech(req: TTSRequest):
    ...
    try:
        ...
        print(f"[Voice] TTS: {req.language} via {model_used}")
        return {
